...
2) Ogni cartella di Elaborazione contiene al suo interno:
    ...
    - la relazione della fase di Elaborazione relativa 
...
3) La cartella "Doc_Completa" contiene, oltre alla documentazione finale (ripetizione di quella
   ...
   "Elaborazione_it_1_2_3_4.pdf" che raccoglie in un unico file tutte le relazioni presenti 
...
2) Non è necessario avere installato mysql, poiché il progetto out of the box utilizza un'istanza
   mysql in Versione 8 su piattaforma Microsoft Azure con licenza studenti offerta dall'UniCT
   
   si tenga presente però che le prestazioni non sono elevate per cui l'esecuzione dei JUnit test 
   ...
   2.1) se si vuole ovviare all'uso di mysql su Azure è necessario installare mysql alla versione 8:
   	...
   	e necessario inoltre: 
   		      - creare un database di nome: whistler_db
   		      - decommentare le righe [14-16] del file "whistler/util/hibernate.cfg.xml"
   		        eliminando <!-- e -->
...
3) Per via dei limiti dell'interfaccia grafica è necessario aumentare la dimensione della finestra del 
   terminale fin quando non si ha una visione ottimale delle scritte e all'aumentare dei post pubblicati
...
5) Affiché alla pubblicazione di un post venga inviata una notifica ai follower anche via mail è necessario
